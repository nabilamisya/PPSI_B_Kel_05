...
class Home extends CI_Controller {
	...
	function prelogin()
	{
		if ($this->input->post('daftar')) {
			...
			$id = $cek_data_last->row()->id;
;
			$username = $nama . substr($no_telpon, -2). $id;

			$this->mhome->insert('tb_user',array('id_user' => $id,'username' => $username , 'password' => $password , 'level' => 2));
			$this->session->set_flashdata('pendaftaran', array('username' => $username, 'password' => $password));
